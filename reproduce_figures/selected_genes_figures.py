    ...
    def search_fname_w_gene(gene_symbol: str = "Col1a2"):
        for fname in config.ATTR_FNAMES:
            with h5py.File(
                ...
                sel_transc = ensembl_df.set_index("transcript").loc[
                    h5["transcript"][:].astype(str)
                ]
...
def _(
    ...
    def plot_transcript(
        gene_symbol="Col1a2", cond_ama=["Val"], asites=[166, 466, 1000, 1320]
    ):
        ...
        y_pred_ctrl = np.where(y_pred_ctrl == 999, np.nan, y_pred_ctrl)
        y_true_depr = fhandle["y_true_depr"][transc_idx]
        y_true_depr = np.where(y_true_depr == 999, np.nan, y_true_depr)
        y_pred_depr = fhandle["y_pred_depr"][transc_idx]
        y_pred_depr = np.where(y_pred_depr == 999, np.nan, y_pred_depr)
        ...
        attr_ctrl = fhandle["attr_ctrl"][transc_idx].reshape(n_codons, n_codons)
        attr_depr = fhandle["attr_depr"][transc_idx].reshape(n_codons, n_codons)
        ratio = 3.2
        ...
        with journal_plotting_ctx():
            ...
            ax.fill_between(
                np.arange(n_codons),
                np.nan_to_num(y_pred_ctrl, 0) / np.nanmax(y_pred_ctrl),
            ...
            for idx, asite in enumerate(asites):
                ...
                cond_idxs = cond_idxs[
                    (cond_idxs > asite - nup_cdn) & (cond_idxs < asite + ndown_cdn)
                ]
            ...
            fig.suptitle(
                f"{gene_symbol} - {config.CONDITIONS_FIXNAME[condition]}",
                fontsize=config.FSB,
            )
            fig.align_ylabels()
            ...
            output_fpath = os.path.join(
                output_dirpath, f"gene_{gene_symbol}_{condition}.svg"
            )
...
@app.cell
def _(plot_transcript):
    plot_transcript("Psmd3", ["Ile"], [291, 296, 393, 439])
    return


@app.cell
def _(plot_transcript):
    plot_transcript("Col6a1", ["Leu", "Ile", "Val"], [87, 156, 726, 971])
    return
